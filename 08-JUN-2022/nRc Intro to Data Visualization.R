...
ggplot(data_complete, aes(weight, hindfoot_length, color=species_id)) +
  geom_jitter()
...
ggplot(data_complete, aes(weight, species_id, color=plot_type)) + geom_point()

# 2. Make a boxplot graph with species_id on the x-axis, weight on the y-axis, 
# and the species_id showing different colours.
ggplot(data_complete, aes(species_id, weight, color=species_id)) +
  geom_boxplot()
